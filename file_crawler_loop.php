...
echo $s;
$query = "SELECT `id` FROM `scraper`.`results` WHERE `results`.`converted` IS NULL LIMIT 1;";
if ($stmt = mysqli_prepare($conn1, $query)) {
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $urlcounter);
    while (mysqli_stmt_fetch($stmt)) {
        echo $urlcounter;
    }
    mysqli_stmt_close($stmt);
}
$query = "UPDATE `scraper`.`results` SET `results`.`converted` = 'Y' WHERE `results`.`id` = '".$urlcounter."';";
echo $query;
if ($stmt = mysqli_prepare($conn1, $query)) {
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
...
$rand = rand(0, 100);
$idcounter = intval($urlcounter) + $rand;
$query = "SELECT `title`, `url`, `host`, `seoterm`, `date` FROM `scraper`.`results` WHERE `results`.`id` = '".$idcounter."';";
echo $query;
if ($stmt = mysqli_prepare($conn1, $query)) {
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $title, $url, $host, $seoterm, $date);
    while (mysqli_stmt_fetch($stmt)) {
    }
    mysqli_stmt_close($stmt);
}
