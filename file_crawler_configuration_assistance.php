...
$query = "SELECT `url` FROM `scraper`.`html_url`;";
if ($stmt = mysqli_prepare($conn, $url)) {
...
*/
    ...
    else 
    {
      echo "Invalid File Format, or captcha"; 
        ...
        $rm_folder = "rm -R -f ".$file_folder_name;
        system($rm_folder, $retrm);
        error_msg('File already NOT scraped!');    
...
$file = file_get_contents("/var/www/html/drive/rss/2017-05-05/output.txt");
$delimiter = "\n";
$splitcontents = explode($delimiter, $file);
//print_r($splitcontents);
foreach($splitcontents as $val) {

$body = file_get_contents($val);
$body = html_entity_decode($body);
$body = strip_html_tags($body);
$body = strip_tags(stripslashes($body));
$body = htmlentities($body);  

        $sql = "INSERT INTO pages(id, body) VALUES('".$val."', '".$body."')";
