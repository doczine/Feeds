...
echo $s;
$query = "SELECT `title`, `url`, `host`, `seoterm`, `date`, `id` FROM `scraper`.`results` WHERE `results`.`id` BETWEEN ".$random." AND ".$random1." or `results`.`id` IS NULL LIMIT 1;";
echo $query;
if ($stmt = mysqli_prepare($conn1, $query)) {
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $title, $url, $host, $seoterm, $date, $urlcounter);
